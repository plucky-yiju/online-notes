...
>- **一句话概括**：适配器模式就是**“包装器”（Wrapper）**。
>- **核心动作**：把一个类的接口变换成客户端所期待的另一种接口。
>- **最佳实践**：优先选择**对象适配器**（组合模式），因为它更加灵活，耦合度更低，避免了 Java 单继承的限制。
>- **转换接口，达成兼容**
...
> **怎么去理解？** 只要你的业务逻辑中出现**“多维度变化”**（即：A 变了不影响 B，B 变了也不影响 A，但它们需要组合在一起工作），就应该想到桥接模式。
>
> **适用场景：**
>
> 1. **JDBC 驱动**：Java 提供了 `Connection` 接口（抽象层），各家数据库厂商（MySQL, Oracle）提供具体的驱动实现（实现层）。通过 `DriverManager` 进行桥接。
> 2. **图形界面库**：比如 AWT/Swing，窗口（Window）是抽象层，而具体的操作系统绘制（Windows PE, Linux X11）是实现层。
> 3. **支付系统**：
>    - 维度 1（支付模式）：密码支付、指纹支付、人脸支付。
>    - 维度 2（支付渠道）：微信、支付宝、银行卡。
>    - 组合：用“人脸”在“微信”支付，用“密码”在“支付宝”支付。
>
> **一句话总结**： **桥接模式就是把继承关系（强耦合）变成组合关系（弱耦合），以此来应对多维度的变化。**
...
> ###  总结
>
> - **一句话概括**：组合模式让你可以把对象组合成**树形结构**，并且能让客户端以**一致的方式**处理个别对象和组合对象。
> - **核心实现**：Composite 类中持有一个 `List<Component>`，并且在业务方法中**递归**调用这个 List 中元素的业务方法。
> - **好处**：极大地简化了客户端代码，客户端不需要写递归逻辑，也不需要写一堆 `if (isLeaf)` 的判断。
